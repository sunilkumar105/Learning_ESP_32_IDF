...
#define HEART_RATE_SVC_UUID 0x5090  /*Heart rate service UUID*/
#define HEART_RATE_CHAR_UUID 0xfcda /*Hear rate characteristics UUID*/
#define HEART_NUM_HANDLE 6
#define NEW_CHAR_UUID 0xfcdb // Example UUID - you should choose an appropriate one
...
struct gatts_profile_inst
{
    ...
    // In the gatts_profile_inst structure, add:
    uint16_t new_char_handle;
    esp_bt_uuid_t new_char_uuid;
    uint16_t new_descr_handle;
    esp_bt_uuid_t new_descr_uuid;
};
...
static uint8_t new_characteristics_val[2] = {0};
...
static esp_attr_value_t new_characteristics_attr = {
    .attr_max_len = 2,
    .attr_len = sizeof(new_characteristics_val),
    .attr_value = new_characteristics_val,
};
...
static void heart_gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event)
    {
        /*Just service registartion*/
    case ESP_GATTS_REG_EVT:
        printf("GATT server register, status %d, app_id %d", param->reg.status, param->reg.app_id);
        ...
        break;
        /*Service has been created now you can add characteristics, like HEART_RATE_CHAR_UUID for heart rate measurement*/
    case ESP_GATTS_CREATE_EVT:
        ...
        heart_property = ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_WRITE;
        ret = esp_ble_gatts_add_char(gl_profile_tab[HEART_PROFILE_APP_ID].service_handle,
                                     &gl_profile_tab[HEART_PROFILE_APP_ID].char_uuid,
        ...
        /*You can add another characteristics here */
        // Add new characteristic
        gl_profile_tab[HEART_PROFILE_APP_ID].new_char_uuid.len = ESP_UUID_LEN_16;
        gl_profile_tab[HEART_PROFILE_APP_ID].new_char_uuid.uuid.uuid16 = NEW_CHAR_UUID;

        ret = esp_ble_gatts_add_char(gl_profile_tab[HEART_PROFILE_APP_ID].service_handle,
                                     &gl_profile_tab[HEART_PROFILE_APP_ID].new_char_uuid,
                                     ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
                                     ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_WRITE,
                                     &new_characteristics_attr, NULL); // You can add an initial value here if needed
        if (ret)
        {
            ESP_LOGE(GATTS_TAG, "add char failed, error code = %x", ret);
        }
        break;
        /*Upper defined characteristics has been added */
    case ESP_GATTS_ADD_CHAR_EVT:
        ...
        if (param->add_char.char_uuid.uuid.uuid16 == HEART_RATE_CHAR_UUID)
        {
        ...
        else if (param->add_char.char_uuid.uuid.uuid16 == NEW_CHAR_UUID)
        {
            printf("New characteristic added, handle %d", param->add_char.attr_handle);
            gl_profile_tab[HEART_PROFILE_APP_ID].new_char_handle = param->add_char.attr_handle;

            // Add descriptor if needed (similar to the heart rate descriptor)
            gl_profile_tab[HEART_PROFILE_APP_ID].new_descr_uuid.len = ESP_UUID_LEN_16;
            gl_profile_tab[HEART_PROFILE_APP_ID].new_descr_uuid.uuid.uuid16 = ESP_GATT_UUID_CHAR_CLIENT_CONFIG;
            esp_ble_gatts_add_char_descr(gl_profile_tab[HEART_PROFILE_APP_ID].service_handle,
                                         &gl_profile_tab[HEART_PROFILE_APP_ID].new_descr_uuid,
                                         ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
                                         NULL, NULL);
        }
        break;
    case ESP_GATTS_ADD_CHAR_DESCR_EVT:
        ...
        if (&param->add_char_descr.descr_uuid == &gl_profile_tab[HEART_PROFILE_APP_ID].descr_uuid)
        {
            gl_profile_tab[HEART_PROFILE_APP_ID].descr_handle = param->add_char_descr.attr_handle;
            hrs_create_cmpl = true;
        }
        break;
        /* When user reads a characteristics, handle read requests */
    case ESP_GATTS_READ_EVT:
        if (param->read.handle == gl_profile_tab[HEART_PROFILE_APP_ID].char_handle)
        {
        ...
        if (param->read.handle == gl_profile_tab[HEART_PROFILE_APP_ID].new_char_handle)
        {
            ESP_LOGI(GATTS_TAG, "new Characteristic read");
            // Handle read of new characteristic
            esp_gatt_rsp_t rsp;
            memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
            rsp.attr_value.handle = param->read.handle;
            rsp.attr_value.len = 5;
            // Set rsp.attr_value.value to your data
            rsp.attr_value.value[0] = 45;
            esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id, ESP_GATT_OK, &rsp);
        }
        break;
        /*When user wants to write a characteristics, handle write requests */
